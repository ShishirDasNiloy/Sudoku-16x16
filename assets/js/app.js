...
const initGameGrid = () => {
    ...
    for (let i = 0; i < Math.pow(CONSTANT.GRID_SIZE,2); i++) {
        ...
        if (row === 3 || row === 7 || row === 11) cells[index].style.marginBottom = '10px';
        if (col === 3 || col === 7 || col === 11) cells[index].style.marginRight = '10px';
...
const hoverBg = (index) => {
    ...
    let box_start_row = row - row % 4;
    let box_start_col = col - col % 4;

    for (let i = 0; i < CONSTANT.BOX_SIZE; i++) {
        for (let j = 0; j < CONSTANT.BOX_SIZE; j++) {
            let cell = cells[16 * (box_start_row + i) + (box_start_col + j)];
    ...
    step = 16;
    while (index + step < 256) {
        cells[index + step].classList.add('hover');
        step += 16;
    }

    step = 1;
    while (index - step >= 16*row) {
        cells[index - step].classList.add('hover');
        step += 1;
    }

    step = 1;
     while (index + step < 16*row + 16) {
        cells[index + step].classList.add('hover');
        step += 1;
    }
}
